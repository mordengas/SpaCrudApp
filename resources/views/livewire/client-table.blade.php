    ...
    <table class="table-auto w-full mb-6">
        ...
        @foreach($clients as $client)
            <tr>
                ...
                <td class="border px-4 py-2">{{ $client->created_at->diffForHumans() }}</td>
                <td class="border px-4 py-2">{{ $client->updated_at->diffForHumans() }}</td>
                <td class="border px-4 py-2"><button class="btn btn-primary" onClick="window.location='{{route('clientsCRUD.edit', $client)}}'">Update</button></td>
                <td class="border px-4 py-2">
                <form action="{{ route('clientsCRUD.destroy',$client->id) }}" method="Post">
                        @csrf
                        @method('DELETE')
